...
@section('title', 'Komplain Karyawan PT Sanema Safari Mulia')
...
<div class="container-fluid">
  ...
  <div class="col-12 mb-2">
    <div class="page-title-box justify-content-between d-flex align-items-center">
      <h4 class="mb-0" style="font-weight: bold;color:black">Data Komplain Karyawan</h4>
      <div class="page-title-right">
        <ol class="breadcrumb m-0">
          <li class="breadcrumb-item"><a href="#" style="text-decoration: none">Data Komplain /</a></li>
        </ol>
      </div>
    </div>
  </div>
  ...
  <h5 class="mt-4 text-center" style="color: #000; font-weight: 300">Seluruh <b style="font-weight: bold">data
      komplain</b> yang ada di Database Aplikasi Penggajian PT Sanema Safari Mulia</h5>
  <!-- DataTales Example -->
      ...
      <div class="table-responsive">
        <table class="table table-hover table-bordered" id="dataComplaint" width="100%" cellspacing="0">
          ...
          <tbody>
            ...
            <tr class="clickable-row" data-href="{{ url('admin/komplain/'.$c->id.'/respon') }}" style="cursor: pointer">
              <td>{{ $c->employee->user->nip }}</td>
              <td>{{ $c->employee->full_name }}</td>
              <td>{{ $c->employee->position->position }}</td>
              <td>{{ $c->subject }}</td>
              <td><span style="display:none;">{{ Carbon\Carbon::parse($c->created_at)->getTimestamp() }}</span>{{
                Carbon\Carbon::parse($c->created_at)->locale('id')->isoFormat('d MMMM YYYY') }}
              </td>
              <td
